...
if __name__ == '__main__':
    ...
    block = FCB(input_channels, output_channels, 256, 256).to('cuda')

    input_tensor = torch.rand(1, input_channels, 256, 256).to('cuda')

    output_tensor = block(input_tensor)

    print(f"è¾“å…¥å°ºå¯¸: {input_tensor.size()}")
    print(f"è¾“å‡ºå°ºå¯¸: {output_tensor.size()}")